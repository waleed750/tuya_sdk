...
class LiquidPullToRefreshState extends State<LiquidPullToRefresh>
  ...
  void didChangeDependencies() {
    ...
    _valueColor = _positionController.drive(
      ColorTween(
        begin: (widget.color ?? theme.colorScheme.secondary).withValues(
          alpha: 0.0,
        ),
        end: (widget.color ?? theme.colorScheme.secondary).withValues(
          alpha: 1.0,
        ),
      ).chain(
  ...
  Future<void> _dismiss(_LiquidPullToRefreshMode newMode) async {
    ...
    // direction change.
    assert(
      newMode == _LiquidPullToRefreshMode.canceled ||
    ...
    return Stack(
      children: <Widget>[
        ...
        AnimatedBuilder(
          ...
          builder: (BuildContext buildContext, Widget? child) {
            return ClipPath(
              clipper: CustomCurveHillClipper(
                centreHeight: height,
                ...
                peakWidth:
                    (_peakHeightUpAnimation.value != 0.0 &&

...
class ConnecitonCubit extends Cubit<ConnectionState> {
  ...
  void _ensurePairingEventsSubscribed() {
    _pairingSub ??= FlutterTuyaSdk.pairingEvents.listen(
      (ev) {
  ...
  Future<void> startWifiScan({
    ...
    try {
      ...
      final token = await FlutterTuyaSdk.getToken(homeId: homeId);
      ...
      final raw = await FlutterTuyaSdk.startConfigWiFi(
        mode: mode,
        ssid: ssid,
  ...
  Future<void> startBleScan({int timeoutSeconds = 30, int? homeId}) async {
    try {
      ...
      if (homeId != null) {
        ...
        try {
          final raw = await FlutterTuyaSdk.smartBlePairing(
            homeId: homeId,
        ...
        _pollTimer = Timer.periodic(const Duration(seconds: 2), (_) async {
          ...
          try {
            final raw = await FlutterTuyaSdk.discoverDeviceInfo();
            final found = _mapSingle(raw, fallbackType: 'ble');
            if (found != null) {
              bleDevice = found;
              emit(OnboardingDevicesFound([bleDevice!], protocol: 'ble'));
  ...
  Future<void> startWifiComboPairing({
    ...
    try {
      ...
      final deviceInfo = await FlutterTuyaSdk.discoverDeviceInfo();
      ...
      final token = await FlutterTuyaSdk.getToken(homeId: homeId);
      final result = await FlutterTuyaSdk.startComboPairing(
        homeId: homeId,
        ssid: ssid,
        password: password,
        token: token,
  ...
  Future<void> startWifiBleComboConfig({
    ...
    try {
      ...
      final autoSub = FlutterTuyaSdk.pairingEvents.listen((ev) {
        final type = ev['type'] as String? ?? '';
      ...
      final raw = await FlutterTuyaSdk.wifiBleComboConfig(
        ssid: ssid,
        password: wifiPassword,
        homeId: homeId,
        productId: productId,
  ...
  Future<void> pairSelected(
    ...
    try {
      ...
      if (type == 'ble' || type.isEmpty) {
        ...
        await FlutterTuyaSdk.pairBleDevice(
          uuid: uuid,
          productId: productId,
          homeId: homeId,
  ...
  Future<void> deleteDevice({required String devId}) async {
    try {
      ...
      await FlutterTuyaSdk.removeDevice(devId: devId);
      // emit(OnboardingIdle());
    } catch (e) {
      emit(OnboardingError(e.toString()));
    }
  ...
  Future<void> _stopAll() async {
    ...
    try {
      await FlutterTuyaSdk.stopAnyPairingOrScan();
    } catch (_) {}

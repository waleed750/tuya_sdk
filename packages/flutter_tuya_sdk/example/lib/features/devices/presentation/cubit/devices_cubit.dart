...
class DevicesCubit extends Cubit<DevicesState> {
  ...
  Future<void> lockDevice(Map<String, dynamic> deviceMap) async {
    ...
    try {
      await FlutterTuyaSdk.controlMatter(
        devId: devId,
        dps: {'1': 0}, // 0 = lock
      );
  ...
  Future<void> unlockDevice(Map<String, dynamic> deviceMap) async {
    ...
    final ifMatter = await FlutterTuyaSdk.checkIsMatter(devId: devId);
    ...
    if (ifMatter) {
      ...
      try {
        await FlutterTuyaSdk.replyRequestUnlock(devId: devId, open: true);
        emit(DeviceStateChanged(deviceId: devId));
      } catch (e) {
        emit(
          DeviceErrorChangedState(
  ...
  Future<void> loadDevices() async {
    ...
    devices.clear();
    emit(DevicesLoading());
    devices = await FlutterTuyaSdk.getHomeDevices(homeId: currentHomeId ?? 0);
    if (devices.isEmpty) {
  ...
  Future<void> listenToRemoteUnlock(String devId) async {
    ...
    await FlutterTuyaSdk.setRemoteUnlockListener(devId);
    ...
    final sub = FlutterTuyaSdk.remoteUnlockEventStream?.listen((event) async {
